...
* `--lotteryall=333` を (`--lottery` の代わりに) 指定した場合は、認証アカウントの333の全競技者に対して当選ポイントを記録する。333以外は抽選になる。`--lottery` オプションと同様に、ポイント加算対象者をMySQLに待ちレコードとして保存する。ここでは実際にポイント加算はされない。このオプションは、これまで「初回コンテストで全員当選にするため」、「通算100回目のコンテスト (2018H1 21節) で333ストア連携全員当選にするため」、「2019H2 24節 クリスマス」、「2020H1 14節, 15節」、「2020H2 1節」、「2020年末てこ入れ月間」、「2025H2 18節マティCR記念472ポイント進呈」等（他にもあったかも）で使用した。全員当選時の進呈ポイント数は `contestmanager/config.js` で指定する。
